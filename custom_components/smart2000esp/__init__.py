...
async def async_setup(hass: HomeAssistant, config: dict):
    _LOGGER.debug("Setting up Smart2000ESP integration")
    return True

async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    _LOGGER.debug("Setting up Smart2000ESP integration entry: %s", entry.as_dict())
    ...
    _LOGGER.debug("Smart2000ESP entry setup completed successfully and update listener registered")
    return True
